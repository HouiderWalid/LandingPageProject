...
const sendApiRequest = function (context) {
  ...
  AxiosConfig.data = Array.isArray(data)
...
function ResponseManager(httpClient, context, foreignApi = false, raw = false) {
  ...
  this.OnSuccess = (success, model, loop = true) => {
    httpClient.then(res => {
        ...
        let data = foreignApi ? response_data : response_data?.data
        ...
        code === 200 || foreignApi
          ...
          : success(raw ? response_data : serializeObject(response_data, Response), message)
          : null
      }
    )
    return this

<template>
  <div class="d-flex justify-content-center align-center" @click="$emit('click')">
    <picture v-if="getImageUrl" :style="{ maxWidth, maxHeight: maxHeight, height: height, width }">
...
export default {
  ...
  computed: {
    ...
    getWebpImage() {
      return this.image?.ImageOtherExtentions?.content?.find?.(i => i?.ImageExtensionsExtension?.content === 'webp')
    },
    getWebpImagePath() {
